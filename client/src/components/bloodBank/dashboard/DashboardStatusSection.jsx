...
const BloodGroupGrid = ({ bloodStocks = [] }) => {
    // console.log('Blood Stocks:', bloodStocks, Array.isArray(bloodStocks)); // Debugging log

    const positiveBloodGroups = ['A+', 'B+', 'O+', 'AB+'];
    const negativeBloodGroups = ['A-', 'B-', 'O-', 'AB-'];

    // Ensure bloodStocks is an array before using reduce
    const stockMap = Array.isArray(bloodStocks)
        ? bloodStocks.reduce((acc, { blood_type, quantity }) => {
              if (blood_type) {
                  acc[blood_type] = quantity ?? 0; // Default to 0 if quantity is null/undefined
              }
              return acc;
          }, {})
        : {};
                            ...
                            <div
                                ...
                                <span
                                    className={`text-lg font-semibold ${
                                        stockMap[group] === 0 ? 'text-red-500' : 'text-green-600'
                                    }`}>
...
const DashboardStatusSection = ({ bloodBankStats, bloodStocks, requestsInfo, revenueInfo }) => {
    return (
                    ...
                    <li className="border p-6 rounded-lg bg-white shadow-md hover:shadow-xl transition-all duration-300">
                        <Link>
                            <div className="h-full flex flex-col gap-4">
                                ...
                                <RequestsInfo requestsInfo={requestsInfo} />
                            </div>
                        </Link>
                    </li>
